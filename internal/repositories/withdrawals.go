...
type WithdrawalsRepository interface {
	...
	// номера заказа.
	Create(ctx context.Context, userID uuid.UUID, orderNumber string, sum float64) (*models.Withdrawal, error)
	...
	// списаний.
	GetUserBalance(ctx context.Context, userID uuid.UUID) (float64, float64, error)
}
